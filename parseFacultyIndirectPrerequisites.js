...
function parsePrerequisiteSections(lines) {
  ...
  if (sectionIndex < 0) {
    throw new Error('Indirect prerequisite results section heading was not found.');
  }
  ...
  if (!Object.keys(prerequisiteCourses).length) {
    throw new Error(
      'The indirect prerequisite results section was found, but no prerequisite course headings were recognized.'
    );
  }
...
export function parseFacultyIndirectPrerequisites(source) {
  ...
  if (!professor) {
    throw new Error('Professor information was not found in the Faculty indirect prerequisite page.');
  }

  if (!course) {
    throw new Error('Course information was not found in the Faculty indirect prerequisite page.');
  }
